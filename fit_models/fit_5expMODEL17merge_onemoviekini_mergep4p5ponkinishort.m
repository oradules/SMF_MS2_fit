...
%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% fits the MODELS 17,18  using the functions exp_fitnessMODELXmergeonekini_mergep4p5ponkinishort.m
...
str=['type, ','OBJ, ','lambda1no,','lambda2no,','lambda3no,',...
    ...
    'k1,','k2, ','k3, ','k4, ','k5, ','k6no, ','k7no, ','k8no, ','k10no, ',...
    'k6hi, ','k7hi, ','k8hi, ','k10hi'];
else
str=['type, ','OBJ, ','lambda1no,','lambda2no,','lambda3no,',...
    'lambda4no,','lambda5no,','A1no,','A2no,','A3no,','A4no,','A5no,','S1no,',...
    'lambda1hi,','lambda2hi,','lambda3hi,',...
    'lambda4hi,','lambda5hi,','A1hi,','A2hi,','A3hi,','A4hi,','A5hi,','S1hi,',...
    'k1,','k2, ','k3, ','k4, ','k5, ','k6no, ','k8no, ','k9no, ','k10no, ',...
    'k6hi, ','k8hi, ','k9hi, ','k10hi'];
end
fprintf(fid,'%s \n ',str);
%line1=1;
...
%%%%%%%%%%%%%%%  initial guess of 2*9+9+4 = 31 pars
%%%%%%%%%%%%%%%  lambda1no,lambda2no,lambda3no,lambda4no,lmabda5no,lambda1hi,lambda2hi,lambda3hi,lambda4hi,lambda5hi,
%%%%%%%%%%%%%%%  A1no,A2no,A3no,A4no,A1hi,A2hi,A3hi,A4hi,k1,k2,k3,k4,k5,k6no,k8no,k9no,k10no,k6hi,k8hi,k9hi,k10hi
%%%%%%%%%%%%%%%   
...
%for MC=1:iter
    ...
    k0(19:31)=k0(19:31).*exp(lf1); %%% ks 
    %k0(32:33)=rand(1,2).*[ppvalno(4)+ppvalno(5),ppvalhi(4)+ppvalhi(5)];
   

if imodel == 17   
    fobj=@(k)exp_fitnessMODEL17mergeonekini_mergep4p5ponkinishort(k, alpha, xno, sno, xhi, shi,  ppvalno,ppvalhi,kinino,kinihi);
else
	fobj=@(k)exp_fitnessMODEL18mergeonekini_mergep4p5ponkinishort(k, alpha, xno, sno, xhi, shi,  ppvalno,ppvalhi,kinino,kinihi);
end
...
num2str(k7no,precision),', ',num2str(k8no,precision),', ',num2str(k10no,precision),', ', ...
num2str(k6hi,precision),', ',num2str(k7hi,precision),', ',...
num2str(k8hi,precision),', ',num2str(k10hi,precision)];
fprintf(fid,'%s \n ',str);
else
...
num2str(k1,precision),', ',num2str(k2,precision),', ',num2str(k3,precision), ', ',...
num2str(k4,precision),', ',num2str(k5,precision),', ',num2str(k6no,precision),', ',...
num2str(k8no,precision),', ',num2str(k9no,precision),', ',num2str(k10no,precision),', ', ...
num2str(k6hi,precision),', ',...
num2str(k8hi,precision),', ',num2str(k9hi,precision),', ',num2str(k10hi,precision)];
fprintf(fid,'%s \n ',str);
else
str=['suboptimal ',', ',num2str(o),', ', ...
num2str(lambda1no,precision),', ',num2str(lambda2no,precision),', ',num2str(lambda3no,precision),', ',...
