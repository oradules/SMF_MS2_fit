...
%%%%% computes non-parametric survival function, computes parameters of
%%%%% five states models; in this version no tat and high tat datasets are
...
%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% fits the MODELS 6,7,8  using the functions exp_fitnessMODELXmergeonekini_mergep4p5ponkinishort.m
...
str=['type, ','OBJ, ','lambda1no,','lambda2no,','lambda3no,',...
    ...
    'k1,','k2, ','k3, ','k4, ','k5, ','k6no, ','k9no, ','k10no, ',...
    'k6hi, ','k9hi, ','k10hi'];
fprintf(fid,'%s \n ',str);
%line1=1;
...
%for MC=1:iter
    ...
    lf1 =  log(fmin)+log(fmax/fmin)*rand(1,11);
    k0(19:29)=k0(19:29).*exp(lf1); %%% ks 
    %k0(30:31)=rand(1,2).*[ppvalno(4)+ppvalno(5),ppvalhi(4)+ppvalhi(5)];
switch imodel
    case 6, fobj=@(k)exp_fitnessMODEL6mergeonekini_mergep4p5ponkinishort(k, alpha, xno, sno, xhi, shi,  ppvalno,ppvalhi,kinino,kinihi);
    case 7, fobj=@(k)exp_fitnessMODEL7mergeonekini_mergep4p5ponkinishort(k, alpha, xno, sno, xhi, shi,  ppvalno,ppvalhi,kinino,kinihi);
    case 8, fobj=@(k)exp_fitnessMODEL8mergeonekini_mergep4p5ponkinishort(k, alpha, xno, sno, xhi, shi,  ppvalno,ppvalhi,kinino,kinihi);
end
...
if ~isinf(sobj) && ~isnan(sobj)
    ...
    10,10,10,10, ...
     0,0,0,0,0,...
     10,10,10,10,...
     10,10,10,10,10,10,10,10,10,10,10],opts);
